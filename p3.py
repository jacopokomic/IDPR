...
# ---------------------------------------------------------------------------------------------- elementi
        ...
        self.kot_help = man.Angle(
            line1 = self.ref,
            line2 = self.kazalec,
            radius = 0.5,
        )

        #def update_kot(kot, dt):

        
        #self.kot.add_updater(update_kot)
        ...
        """self.lok1 = man.Arc(
            start_angle = 0,
            angle = (1/4) * man.PI - (1/24) * man.PI,
            radius = 1,
            arc_center = self.krog.get_center(),
        )

        self.kot = man.Arrow(
            start = [self.lok1.get_end()[0] + 1, self.lok1.get_end()[1] - 1, 0],
            end = [self.lok1.get_end()[0] - 0.09, self.lok1.get_end()[1] + 0.09, 0],
            max_tip_length_to_length_ratio = 0.12,
            stroke_width = 0,
            buff = 0,
        )"""
        ...
        """self.lok2 = man.Arc(
            ...
            angle = man.PI - (1/24) * man.PI,
            radius = 0.5,
            arc_center = self.krog.get_center(),
        )

        self.kotna_hitrost = man.Arrow(
            start = [self.lok2.get_end()[0] - 1, self.lok2.get_end()[1] - 1, 0],
            end = [self.lok2.get_end()[0] + 0.07, self.lok2.get_end()[1] + 0.07, 0],
            max_tip_length_to_length_ratio = 0.12,
            stroke_width = 0,
            buff = 0,
        )
...
# ---------------------------------------------------------------------------------------------- prikaz animacij
        ...
        self.play(man.Write(self.F0), man.Write(self.sinus), man.Write(self.phi))
        self.wait() 
        ...
        self.play(man.GrowFromPoint(self.kot_help, [4.5, 0, 0]))
        self.remove(self.kazalec_help, self.povezava_help, self.kot_help)
        self.add(self.kazalec, self.povezava, self.kot)
        self.wait()
        ...
        self.play(self.F.animate.set_value((19/4) * man.PI),
                  run_time = 5.5, rate_func = man.linear)
        ...
        self.play(self.F.animate.set_value((25/4) * man.PI),
                  run_time = 3, rate_func = man.linear)
